# 接手入口：OpenScope Android 0.4.10 本地开发版

**2026-09-24 刷新按钮移除：**0.4.10 使用 `versionCode=14`。内置网页不再显示右上角刷新悬浮按钮，错误提示改为返回重试；视频全屏入口和系统返回行为保持 0.4.9 的实现。Verify 通过 58 项测试，x86_64 APK 在模拟器覆盖安装；本次模拟器 DNS 故障未能复测平台内容。ARM64 与 x86_64 安装包、校验值见 [docs/release-0.4.10.md](docs/release-0.4.10.md)。

**2026-09-24 内置网页全屏与导航：**0.4.9 使用 `versionCode=13`。移除占高的原生网页顶栏，普通页面只保留角落里的刷新；手机视频页可从右下角进入横屏全屏，全屏不显示退出按钮或任何 OpenScope 悬浮控件，系统返回键/手势退出并恢复先前方向。平板视频横屏也隐藏系统栏。最终 x86_64 APK 在模拟器验收手机进入/退出及平板横屏；ARM64 与 x86_64 安装包、校验值及限制见 [docs/release-0.4.9.md](docs/release-0.4.9.md)。

**2026-09-24 手机布局与本地 APK：**0.4.8 使用 `versionCode=12`。普通搜索首页两处四平台按钮均改为单行四等宽，不再裁切第四个抖音按钮。抖音独立视频页隐藏桌面侧栏和站内顶栏，互动区四项等宽、转发图标对齐；最终 x86_64 APK 在模拟器验证竖屏、横屏及退出恢复。BewlyBewly 原版扩展在 Android WebView 中实测无法直接运行，按用户决定不内置。ARM64 与 x86_64 APK、校验值及限制见 [docs/release-0.4.8.md](docs/release-0.4.8.md)。
